...
def shade_gouraud(vertsp, vertsn, vertsc, bcoords, cam_pos, mat, lights, light_amb, light_type, X):
    """
    ...
    # Calculates the color of each vertex using the full light model
    for i in range(3):
        normal = np.array([vertsn[:, i]]).T
        color = np.array([vertsc[:, i]]).T
        vertsc[:, i] = light(bcoords, normal, color, cam_pos, mat, lights, light_amb, light_type).T[0]
    ...
    for y in range(ymin, ymax):
        ...
        A, B, A_color, B_color = calculate_colors(active_points, vertsp, vertsc)
